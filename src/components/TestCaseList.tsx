  ...
  const allOrSpecific = (
    <div className="test-case-selector">
      <button
        ...
        onClick={(): void => onButtonClick(problem.name, 'all', true)}
      >
        All Test Cases
      </button>
      <button
        type="button"
        className={showTestCases ? 'selected-btn' : ''}
        onClick={(): void => onButtonClick(problem.name, 'none', true)}
      >
        Specific Test Cases
      </button>
    </div>
      ...
      <div className="specific-case-list">
        {shouldFlag ? (
          <p className="bad-choice">
            <em>Select at least one test case</em>
          </p>
        ) : null}
        {allOrSpecific}
        <ul className="specific-cases">
          {problem.testCases.map((testCase, i) => {
            ...
            return (
              <li
                key={`${problem.name} ${testCase.inputs.join(
                  ','
              ...
              >
                <button
                  ...
                  onClick={(): void =>
                    onButtonClick(problem.name, i, !isSelected)
                  }
                >
                  {`[${testCase.outputs.join(', ')}] = ${
                    problem.name
