...
export async function getTAs(): Promise<TA[]> {
  return fetch('./api/tas.php').then(resp => resp.json() as Promise<TA[]>);
}

export async function getHomeworks(): Promise<ShallowHomework[]> {
  return fetch('./api/homework.php').then(
    resp => resp.json() as Promise<ShallowHomework[]>
  );
}

export async function getHomework(num: number): Promise<Homework> {
  return fetch(`./api/homework.php?num=${num}`).then(
    resp => resp.json() as Promise<Homework>
  );
}
